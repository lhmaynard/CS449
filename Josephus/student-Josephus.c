...
void printStudent( void * data, int mode )
{
	...
	STUDENT *person = (STUDENT *)data;

	if(mode==BRIEF)printf("%s ", person->andrewID);
	else printf("%s %s %i %f %s ", person->andrewID, person->name, person->yrOfGrad, person->gpa, person->major);
...
int compareStudent( void * data1, void *data2 )
{
	...
	STUDENT *dat1 = (STUDENT *)data1;
	STUDENT *dat2 = (STUDENT *)data2;
	char *word1 = dat1->andrewID;
	char *word2 = dat2->andrewID;

	unsigned int x;
	unsigned int len = strlen(word1);
	if(strlen(word2)>len) len = strlen(word2);
	for(x=0; x<len; x++){

	if(word1[x]!=word2[x]) return 1;
	}
	return 0;
